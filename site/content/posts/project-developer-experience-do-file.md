...
> [here](/tags/pdx/). A fully functional example with all examples from this post that can be used as a template is
...
There may be a `README.MD` somewhere with some information about some commands that you can run to achieve those goals, 
but this information tends to get outdated very fast. Looking at the state-of-the-art approach towards deploying 
infrastructure, which is pouring all the information that is needed to create it in code we can try to apply the same 
pattern to all the glue code that is needed to work with our project.

This rather abstract idea can be materialized as a simple script that serves as an entrypoint for all tasks that are 
...
Before we dive into the structure of this `do` scripts we have to make the always painful decision which language to 
use to implement it. Keeping in mind that we may be confronted with a wide range of development environments that 
vary in architecture (AMD64, ARM64, ...) and operating systems (Linux, Windows, OSX, ...) or even worse both we need 
to make a decision that supports most of them or at least the ones that are relevant to our situation.

Since the introduction of the Linux subsystem for Windows, the bash shell is a reasonable lowest common denominator to
chose as a scripting language that works across all major operating systems.

Unfortunately bash scripts tend to evolve into an unmaintainable mess of `sed`, `awk` and really akward regular 
expressions really fast, so another good contender could be Python which also has a solid support across all major 
operating systems. 

Whatever you choose, the purpose of the `do` file is to put all steps needed to interact with the project in code.
Ideally this code can also be used in the CI/CD environment, so we keep the way the repository is built, tested and 
deployed int the CI close the local machine, making it easier to debug potential problems in the CI.
...
using the [Python shebang](https://realpython.com/python-shebang/) this would confront us with several issues. First we might need some Python packages 
(PIPs) for our do file that need to be to fetched first and second we most likely don't want to use the systems Python 
installation for this as this would create endless possibilities for errors because of conflicts with packages that are
already installed system-wide. We will use [Python venv](https://docs.python.org/3/library/venv.html) to create a dedicated Python environment for our `do.py` 
file and provide a little bash based starter the bootstraps and executes the created `venv` environment.
...
Having common patterns here helps you navigate and find your way around multiple `do` in case you have more than one
repository. 
...
To achieve this in Python we can make use of a command line library like for example [click](https://click.palletsprojects.com/en/8.1.x/) which lets us not 
...
After the previously explained bootstrap step vi the `do` file, the commands defined via click are directly callable:
...
Ideally the tasks from the `do` file can also be used in CI/CD system. Keeping that in mind we should never assume that 
...
The same applies for directory changes, that should always be done in a subshell to ensure we are not messing with the
...
CI/CD and our local machine. We should try to keep those differences as small as possible but if needed for most 
CI/CD systems we can decide where we are executed depending on the de-facto-standard `CI` environment variable. 
...
expressions or tools like `aws` and `sed`. If datasource offers a structured machine parsable format like JSON 
we should consume that with the appropriate tooling. For commands that do not offer a structured output tools 
like [jc](https://kellyjonbrazil.github.io/jc/) can help us to make the output easy parsable.
...
If a tools does not expose JSON, `jc` might be able to convert it
...
fail with a meaningful error message as fast as possible. If possible the error message should not only say what is 
...
Especially import for Bash is to check mandatory arguments and validate them before they are used
...
To make the developers life easier we should at least consider how he is supposed to install the needed software to 
execute the `do` file. In general, we do not want to mess with the developers system to install software, or make any 
assumptions about how the system is configured. So if we need a specific command that is generally available in the 
package managers of the operating systems we need to support, a small hint on how to install it already goes a long way.
...
Sometimes when a software package is not commonly available we might want to go the extra mile and conveniently 
download it so the developer does not have to hassle with installing software and making it available on the `PATH`.
If we do this, we should again avoid littering the developers system and keep the changes local to the repository. 
Also, we must not trust anything we just fetched from the internet and always verify the checksum of everything 
we download.
...
We should factor in that our `do` might get cancelled or interrupted at any time. This has implications for tasks that 
for example decompress some files and are interrupted mid-compress leaving us with a partial state. To avoid this 
...
Unfortunately for bash there is no reliable to automatically document the tasks for the user, the easiest way is to 
create a help page that needs to be manually updated everytime a task is changed.
...
available in some form of password manager it is a good practice to directly read them from there in the `do` file. 
This avoids forcing the user to prepare the environment by himself which could lead to secrets being accidentally 
added to the users shell history. We have to keep in mind though that password managers often need interactive steps 
from the user to unlock, so we need to provide a way for the CI/CD systems to provide secrets as well which commonly 
...
also prevents us from accidentally exposing secrets to the systems `tmp` folder in case we forgot to set the correct 
...
a cleanup-task that removes all those ephemeral data and resets your repository to a clean known state
...
Important to keep in mind is not to go overboard with complex algorithms in bash. If things get to complicated, or you
need to talk to third-party APIs, Python or Ruby may be a more sensible choice. You can also use a mixture and take 
bash to bootstrap Python and orchestrate some simple command calls, and use Python for all other more complex tasks.
